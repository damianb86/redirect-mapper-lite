...
REMOTE_ENV_FILE=.env ./deploy-production.local.sh
...
production server. Production runtime variables such as `OPENAI_API_KEY` must be
present in the remote `.env` file because that is the env file used by the server
deploy.
